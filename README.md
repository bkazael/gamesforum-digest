...
`source_health.py` עוקב אחרי כמה ריצות שבועיות רצופות כל מקור חוזר עם
אפס תוצאות. אחרי שתי ריצות רצופות כאלה, `weekly-digest.yml` מכשיל את
עצמו בכוונה **אחרי** שהפרק כבר פורסם — כדי שמקור שקרס בשקט (feed
שנעלם, שינוי במבנה עמוד) לא יישאר בלתי מורגש.
